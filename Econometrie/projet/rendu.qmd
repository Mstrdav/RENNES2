...
for (i in 1:ncol(data_numeric)) {
    p <- ggplot(data_numeric, aes_string(x = data_numeric[[i]])) +
        ...
        labs(x = "Fréquence" , y = nomsdevar[i]) +
        theme_minimal() +
...
On visualise deux pics de surreprésentation (autour de 25 et 55) dans l'histogramme du surpoids et une certaine absence des individus dans les valeurs moyennes (entre 30 et 50). On peut voir une concentration des espérances de vie au delà de 70 ans.

Ces histogrammes ne nous sont d'une grande utilité si ce n'est pour dire que la corruption politique, le surpoids et l'espérance de vie ne semblent pas suivre des lois classiques. Les nuages de point et les boxplots nous seront peut-être plus utiles pour analyser ces variables.
...
Figure 9 : Nuages de points les plus concentrés avec et sans les individus les plus aberrants
:::
...
Figure 11 : Boxplots des variables explicatives
:::
...
::: {#tab2 .fig}
Tableau 2 : Résumés de reglin et reglog
...
::: {#tab3 .fig}
Tableau 3 : VIF des variables du modèle complet (reglog)
...
::: {#tab4 .fig}
Tableau 4 : VIF des variables de reglog près suppression de la consommation d'énergie
...
::: {#tab5 .fig}
Tableau 5 : Résumé du modèle optimisé par AIC
...
::: {#tab6 .fig}
Tableau 6 : Résumés du modèle final et du modèle avec interaction
...
::: {#tab7 .fig}
Tableau 7 : Test d'anova entre le modèle final et celui avec interaction
...
::: {#tab8 .fig}
Tableau 8 : Tableau de synthèse des diagnostics
...
-   **Levier vs Résidus :** La **Somalie** se distingue par un fort effet de levier (probablement dû à ses valeurs extrêmes en scolarisation, ou des valeurs manquantes trop lissées par notre pré traitement), mais son résidu reste modéré. **Oman**, le **Botswana** et la **Jordanie** ont des résidus plus importants, suggérant que le modèle explique moins bien leur niveau de bonheur spécifique (facteurs géopolitiques ou culturels non capturés).
...
::: {#tab9 .fig}
Tableau 9 : Résultats finaux du modèle économétrique
