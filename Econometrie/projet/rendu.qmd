...
stargazer(data_numeric, type = "text", title = "Statistiques descriptives", digits = 2,
...
Ici, retirer les individus les plus aberrants ne permet pas pour autant d'obtenir une linéarité de la relation entre l'indice de bonheur et les variables étudiées. Au contraire : tout comme les dépenses militaires, ces variables ne semblent pas corrélées à notre score de bonheur.
...
for (i in c(2,3,5,7,9,11)) {
  p <- ggplot(data, aes_string(y=data[[i]])) +
    geom_boxplot(notch=FALSE, outlier.colour="salmon1", outlier.shape=8,
                outlier.size=4) +
...
Les résultats indiquent une multicolinéarité sévère pour le PIB par habitant (**16.82**) et la consommation d'énergie (**13.01**), ainsi qu'une valeur limite pour l'espérance de vie (**5.24**). 
Bien que le PIB présente le VIF le plus élevé, il constitue la variable économique de référence. La consommation d'énergie (`KWH_pp_pc`) étant une proxy de l'industrialisation très corrélée au PIB, sa suppression est la correction logique pour réduire la redondance sans perdre l'information sur la richesse. Nous retirons donc `KWH_pp_pc`.
...
Les tests sont concluants et valident toutes les hypothèses des Moindres Carrés Ordinaires :
...
**Analyse visuelle :**
Les graphiques confirment la robustesse du modèle.
...
4.  **Limites du modèle :** Bien que les diagnostics (homoscédasticité, normalité) soient globalement satisfaisants après correction ou transformation, des limites subsistent.
...
*   World Bank (GDP, Lifespan, Corruption, etc.)
*   Our World in Data (Energy, Military, Population)
