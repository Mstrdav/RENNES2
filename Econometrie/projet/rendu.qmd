...
Comme précédemment, on conserve ici l'hypothèse nulle : il n'y a pas d'autocorrélation des résidus.
...
summary(reglog) #le niveau-log a un R2 ajusté un peu supérieur au niveau-niveau
...
```{r echo=FALSE}
#Distance de Cooks
plot(reglog, which = 4, main = "Distances de Cook")

```

```{r echo=FALSE}
#En mode chasse d'étoile
summary(lm(Happiness ~ log(GDP_per_Capita), data = data))
print("#########################      OCDE     ##########################")
summary(lm(Happiness ~ log(GDP_per_Capita)  + OCDE, data = data))
print("########################      Lifespan     ########################")
summary(lm(Happiness ~ log(GDP_per_Capita) + Lifespan , data = data))
print("####################      OCDE + Lifespan    #####################")
summary(lm(Happiness ~ log(GDP_per_Capita) + Lifespan + OCDE, data = data))
```
