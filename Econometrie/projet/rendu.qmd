...
ggplot(data_ss_singapour, aes_string(x="Population_Density", y="Happiness")) +
    geom_point() +
    geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
    labs(x="Linéarité de la densité de population sans Singapour", y="Happiness") +
    theme_minimal()
...
ggplot(data_ss_somalie, aes_string(x="Primary_School_Enrollment", y="Happiness")) +
    geom_point() +
    geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
    labs(x="Linéarité des inscriptions à l'école primaire sans la Somalie", y="Happiness") +
    theme_minimal()

data_ss_pak_usa = data[-which(data$Code == "PAK" |data$Code == "USA" ),]
ggplot(data_ss_pak_usa, aes_string(x="Net_Migration", y="Happiness")) +
    geom_point() +
    geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
    labs(x="Linéarité de la migration sans les USA ni le Pakistan", y="Happiness") +
    theme_minimal()
```
...
ggplot(data_PIB_log, aes_string(x="GDP_per_Capita", y="Happiness")) +
    geom_point() +
    geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
    labs(x="Linéarité du logarithme du PIB par habitant", y="Happiness") +
    theme_minimal()


data_KWH_log = data
data_KWH_log$KWH_pp_pc  = log(data_KWH_log$KWH_pp_pc)
ggplot(data_KWH_log, aes_string(x="KWH_pp_pc", y="Happiness")) +
    geom_point() +
    geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
    labs(x="Linéarité du logarithme de la consommmation énergétique", y="Happiness") +
    theme_minimal()


data_migration = data
data_migration$Net_Migration  = log(data_migration$Net_Migration)
ggplot(data_migration, aes_string(x="Net_Migration", y="Happiness")) +
    geom_point() +
    geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) +
    labs(x="Linéarité du logarithme de la migration", y="Happiness") +
    theme_minimal()
```
...
# ols_plot_resid_hist(reg1) # en commentaire parce que ça la trouve pas ??

#Residual vs fitted
plot(reg1, which = 1) #On voit une forme de trompette aux résidus
#normal QQplot
plot(reg1, which=2) #SI NORMALITE DES RESIDUS : ILS SERAIENT SUR LA LIGNNE

```

Le graphique peut nous faire douter de la normalité des résidus.
